...
import { Position, Side, AssetClass } from '../types';
...
export const PositionsTable: React.FC<PositionsTableProps> = ({ positions, assetClass }) => {
  return (
    <div className="overflow-x-auto">
          ...
          <tr>
            ...
            {assetClass === 'SPOT' ? (
              ...
              <>
                 ...
                 <th className="px-4 py-3 font-medium text-right">Gamma</th>
                 <th className="px-4 py-3 font-medium text-right">Vega</th>
              </>
            )}
              ...
              <tr key={`${pos.symbol}-${idx}`} className="hover:bg-gray-800/30 transition-colors">
                <td className="px-4 py-3 font-bold text-gray-200">
                  <div className="flex items-center gap-2">
                    ...
                    {pos.symbol} {pos.productType && <span className="text-gray-500 font-normal text-[10px] ml-1 px-1 border border-gray-700 rounded">{pos.productType}</span>}
                  </div>
                </td>
                ...
                {assetClass === 'SPOT' ? (
                  ...
                  <>
                    <td className="px-4 py-3 text-right text-gray-200 font-mono">
                      {pos.strikePrice?.toFixed(4)}
                    </td>
                    <td className="px-4 py-3 text-right font-mono text-blue-400">
                      <LiveTicker value={pos.delta || 0} format={(v) => v.toFixed(1)} />
                    </td>
                    <td className="px-4 py-3 text-right font-mono text-orange-400">
                      <LiveTicker value={pos.gamma || 0} format={(v) => v.toFixed(2)} />
                    </td>
                    <td className="px-4 py-3 text-right font-mono text-purple-400">
                      <LiveTicker value={pos.vega || 0} format={(v) => v.toFixed(1)} />
                    </td>
                  </>
                )}
      ...
      {positions.length === 0 && (
        <div className="p-8 text-center text-gray-600 text-sm">
          No active {assetClass === 'SPOT' ? 'FX Spot' : 'Option'} positions.
        </div>
      )}
    </div>
  );
